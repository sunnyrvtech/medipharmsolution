...
class HeaderSetting extends Component {
  ...
  handleMenuSubmit() {
    ...
    if (this.state.menu_type == "menu_with_sub") {
      ...
      header_menu[menu_index].sub_menu.push({
  ...
  handleSocialHolderChange = idx => evt => {
    const new_social_links = this.state.header_social_links.map(
      (socialLink, sidx) => {
  ...
  handleAddMoreSocialHolder = () => {
    this.setState({
      header_social_links: this.state.header_social_links.concat([
        { class: "", link: "" }
      ])
    });
  };
  handleRemoveSocialHolder = idx => () => {
    this.setState({
      header_social_links: this.state.header_social_links.filter(
        (s, sidx) => idx !== sidx
      )
    });
  };
  handleProgramHolderChange = idx => evt => {
    ...
    const new_header_programs = this.state.header_programs.map((program, sidx) => {
      if (idx !== sidx) return program;
      return { ...program,name:name, slug: evt.target.value};
    });
    this.setState({ header_programs: new_header_programs });
  };
  handleProgramHolder = () => {
    this.setState({
      header_programs: this.state.header_programs.concat([{ slug: "" }])
    });
  };

  handleRemoveProgramHolder = idx => () => {
    this.setState({
      header_programs: this.state.header_programs.filter((s, sidx) => idx !== sidx)
    });
  };
  handleSubmit(e) {
    e.preventDefault();
    const content = {
      ...
      header_programs: this.state.header_programs,
    ...
    this.props.getSettingBySlug(slug, this.props.history).then(response => {
      if (response) {
        this.setState({
          ...
          header_programs:
            response.content.header_programs != undefined
              ? response.content.header_programs
              : [],
  ...
  render_html() {
    const { pages, header_menu,categories,header_programs } = this.state;
    return (
      <div className="container setting">
              ...
              <div className="row mb-3">
                <div className="col-md-6">
                  <div className="card">
                    ...
                    <div className="card-body">
                      <ul className="tree">
                        {header_menu.length != undefined &&
                          header_menu.map((menu, index) => (
                            <li className="branch" key={index}>
                              {menu.sub_menu != undefined ? (
                                <div>
                                  <span
                                    className="indicator fa fa-plus-circle"
                                    onClick={this.handleAddMenuHolder.bind(
                                      this,
                                      "sub_menu",
                                      index
                                    )}
                                  ...
                                  {menu.name}
                                  <span
                                    className="indicator fa fa-times-circle"
                                    onClick={this.handleRemoveMenuHolder.bind(
                                      this,
                                      index
                                    )}
                                  />
                                    ...
                                    <ul>
                                      {menu.sub_menu.map((sub_menu, k) => (
                                        <li key={k}>
                                          ...
                                          <span
                                            className="indicator fa fa-times-circle"
                                            onClick={this.handleRemoveSubMenuHolder.bind(
                                              this,
                                              index,
                                              k
                                            )}
                                ...
                                <div>
                                  <span className="indicator fa fa-pagelines" />
                                  {menu.name}
                                  <span
                                    className="indicator fa fa-times-circle"
                                    onClick={this.handleRemoveMenuHolder.bind(
                    ...
                    <div className="card-body">
                      <div className="fontawe text-center mb-2">
                        <a
                          ...
                          target="_blank"
                        >
                          Get social Icon class here
                        </a>
                      </div>
                      {this.state.header_social_links != undefined &&
                        this.state.header_social_links.map(
                          (socialLink, idx) => (
                              ...
                              <div className="input-group mb-2">
                                <div className="col-md-4">
                                  <input
                                    ...
                                    onChange={this.handleSocialHolderChange(
                                      idx
                                    )}
                          ...
                          <div className="" key={idx}>
                            <div className="input-group-append mb-2">
                              <select
                                ...
                                className="form-control"
                                value={program.slug}
                                onChange={this.handleProgramHolderChange(idx)}
                              >
        ...
        <Modal isOpen={this.state.modal} className="">
                ...
                <div className="col-md-12">
                  <div className="form-group">
                    ...
                    <select
                      name="slug"
                      className="form-control"
                      onChange={this.handleInputSelectChange.bind(this)}
                    >
                      <option value="">Select Page</option>
                      {pages != undefined &&
                        pages.map((page, i) => (
                          <option value={page.slug} key={i}>
                            {page.name}
                          </option>
                        ))}
                    </select>
          ...
          <ModalFooter>
            <button
              type="button"
              onClick={this.handleMenuSubmit.bind(this)}
              className="btn btn-info"
            >
              Submit
            </button>
...
export default connect(
  null,
  { updateSetting, getSettingBySlug, getstaticPages,getCategories }
